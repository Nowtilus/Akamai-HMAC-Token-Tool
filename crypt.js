...
var CryptoJS = CryptoJS || function (h, i) {
  var e = {},
    ...
    l = f.Base = function () {
      function a() { } return {
        extend: function extend(j) {
          a.prototype = this; var d = new a(); j && d.mixIn(j); d.$super = this; return d;
    ...
    k = f.WordArray = l.extend({
      init: function init(a, j) {
        a = this.words = a || []; this.sigBytes = j != i ? j : 4 * a.length;
        ...
        var j = this.words,
          d = a.words,
          c = this.sigBytes,
          a = a.sigBytes; this.clamp(); if (c % 4) for (var b = 0; b < a; b++) {
            j[c + b >>> 2] |= (d[b >>> 2] >>> 24 - 8 * (b % 4) & 255) << 24 - 8 * ((c + b) % 4);
          } else if (65535 < d.length) for (b = 0; b < a; b += 4) {
            j[c + b >>> 2] = d[b >>> 2];
          } else j.push.apply(j, d); this.sigBytes += a; return this;
      }, clamp: function clamp() {
        var a = this.words,
          b = this.sigBytes; a[b >>> 2] &= 4294967295 << 32 - 8 * (b % 4); a.length = h.ceil(b / 4);
      }, clone: function clone() {
        var a = l.clone.call(this); a.words = this.words.slice(0); return a;
      ...
      reset: function reset() {
        ...
        var b = this._data,
          ...
          c = h.min(4 * a, c); if (a) {
            ...
            } f = d.splice(0, a); b.sigBytes -= c;
          } return k.create(f, c);
      }, clone: function clone() {
        var a = l.clone.call(this); a._data = this._data.clone(); return a;
      }, _minBufferSize: 0
    }); f.Hasher = b.extend({
      init: function init() {
        this.reset();
  ...
  var i = CryptoJS,
    ...
    o = []; (function () {
      ...
      l = l.SHA256 = e.extend({
        ...
        }, _doProcessBlock: function _doProcessBlock(e, f) {
          for (var b = this._hash.words, g = b[0], a = b[1], j = b[2], d = b[3], c = b[4], h = b[5], l = b[6], k = b[7], n = 0; 64 > n; n++) {
            if (16 > n) m[n] = e[f + n] | 0; else {
              var i = m[n - 15],
                p = m[n - 2]; m[n] = ((i << 25 | i >>> 7) ^ (i << 14 | i >>> 18) ^ i >>> 3) + m[n - 7] + ((p << 15 | p >>> 17) ^ (p << 13 | p >>> 19) ^ p >>> 10) + m[n - 16];
            } i = k + ((c << 26 | c >>> 6) ^ (c << 21 | c >>> 11) ^ (c << 7 | c >>> 25)) + (c & h ^ ~c & l) + o[n] + m[n]; p = ((g << 30 | g >>> 2) ^ (g << 19 | g >>> 13) ^ (g << 10 | g >>> 22)) + (g & a ^ g & j ^ a & j); k = l; l = h; h = c; c = d + i | 0; d = j; j = a; a = g; g = i + p | 0;
          } b[0] = b[0] + g | 0; b[1] = b[1] + a | 0; b[2] = b[2] + j | 0; b[3] = b[3] + d | 0; b[4] = b[4] + c | 0; b[5] = b[5] + h | 0; b[6] = b[6] + l | 0; b[7] = b[7] + k | 0;
...
(function () {
  var h = CryptoJS,
    i = h.enc.Utf8; h.algo.HMAC = h.lib.Base.extend({
      init: function init(e, f) {
        e = this._hasher = e.create(); "string" == typeof f && (f = i.parse(f)); var h = e.blockSize,
          k = 4 * h; f.sigBytes > k && (f = e.finalize(f)); for (var o = this._oKey = f.clone(), m = this._iKey = f.clone(), q = o.words, r = m.words, b = 0; b < h; b++) {
            q[b] ^= 1549556828, r[b] ^= 909522486;
          } o.sigBytes = m.sigBytes = k; this.reset();
      }, reset: function reset() {
        ...
        var f = this._hasher,
          e = f.finalize(e); f.reset(); return f.finalize(this._oKey.clone().concat(e));
      }
    });
})();
